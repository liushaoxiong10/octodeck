...
const SHARED_TMP =
  ...
  (() => {
    const d = fs.mkdtempSync(path.join(os.tmpdir(), 'octodeck-routes-plugins-'));
...
// flipped per-test via OCTODECK_TEST_USER_ROLE env var.
vi.mock('../src/middleware/auth.ts', () => ({
  authMiddleware: async (c: any, next: any) => {
    c.set('user', {
      id: process.env.OCTODECK_TEST_USER_ID ?? 'alice',
      username: 'alice',
      role: process.env.OCTODECK_TEST_USER_ROLE ?? 'member',
      permissions: [],
    });
    return next();
  },
...
const USER = 'alice';
process.env.OCTODECK_TEST_USER_ID = USER;
...
beforeEach(() => {
  delete process.env.OCTODECK_TEST_USER_ROLE;
