...
beforeEach(() => {
  tmpDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'octodeck-mat-'));
});
...
describe('materializeUserRuntime', () => {
  ...
  test('builds runtime tree from catalog snapshot with isolated inodes', () => {
    ...
    expect(
      fs.existsSync(path.join(rtDir, '@octodeck-runtime-markers')),
    ).toBe(false);
    const markerPath = path.join(
      getUserSnapshotsDir(USER), 'sha256-aaa', '@octodeck-runtime-markers', 'mp1', 'p1.json',
    );
