...
async function executePipeline(
  ...
  for (let index = 0; index < team.roles.length; index += 1) {
    ...
    if (role.parallelGroup) {
      ...
      const chainResults = await Promise.all(chains.map((chain) => executeRoleChain(team, chain.roles, input.prompt, roleResults, runner, events, state)));
      for (const results of chainResults) roleResults.push(...results);
      const failed = chainResults.flat().find((result) => result.status === 'error');
      if (failed) return summarize(team, 'error', roleResults, events, state, failed.error || failed.result);
      index += groupedRoles.length - 1;
      continue;
    }
    const phase = roleLooksLikeTest(role) ? 'test' : index === team.roles.length - 1 ? 'finalize' : 'work';
    const result = await runRole(team, role, input.prompt, phase, roleResults, runner, undefined, state);
    roleResults.push(result);
    events.push({ kind: 'role', roleId: role.id, phase });
    if (result.status === 'error') return summarize(team, 'error', roleResults, events, state, result.error || result.result);
    ...
    if (roleLooksLikeTest(role) && isFailedTestResult(result.result) && feedbackUsed < maxFeedbackIterations) {
      ...
      const feedbackResult = await runRole(team, target, input.prompt, 'feedback', roleResults, runner, result.result, state);
      roleResults.push(feedbackResult);
      events.push({ kind: 'role', roleId: target.id, phase: 'feedback' });
      if (feedbackResult.status === 'error') return summarize(team, 'error', roleResults, events, state, feedbackResult.error || feedbackResult.result);
      const retest = await runRole(team, role, input.prompt, 'feedback', roleResults, runner, feedbackResult.result, state);
      ...
      if (retest.status === 'error' || isFailedTestResult(retest.result)) {
        return summarize(team, retest.status === 'error' ? 'error' : 'success', roleResults, events, state, retest.error);
      }
    }
  }
...
async function executeParallel(
  ...
  const roleResultsByChain = await Promise.all(chains.map(async (chain) => {
    ...
    return executeRoleChain(team, chain.roles, input.prompt, [], runner, events, state);
  }));
  const roleResults = roleResultsByChain.flat();
  const failed = roleResults.find((result) => result.status === 'error');
  return summarize(team, failed ? 'error' : 'success', roleResults, events, state, failed?.error);
}
...
async function executeFlatParallel(
  ...
  const roleResults = await Promise.all(team.roles.map(async (role) => {
    ...
    const result = await runRole(team, role, input.prompt, 'work', [], runner, undefined, state);
...
async function executeRoleChain(
  ...
  for (const role of roles) {
    ...
    const result = await runRole(team, role, prompt, roleLooksLikeTest(role) ? 'test' : 'work', previousResults, runner, undefined, state);
...
async function executeLeaderWorker(
  ...
  const roleResults: AgentTeamRoleResult[] = [];
  const events: AgentTeamExecutionEvent[] = [];
  if (!lead) return summarize(team, 'error', roleResults, events, state, 'team has no lead role');

  const plan = await runRole(team, lead, input.prompt, 'plan', roleResults, runner, undefined, state);
  roleResults.push(plan);
  events.push({ kind: 'role', roleId: lead.id, phase: 'plan' });
  if (plan.status === 'error') return summarize(team, 'error', roleResults, events, state, plan.error || plan.result);
  ...
  const workerResults = await Promise.all(workers.map(async (worker) => {
    ...
    const result = await runRole(team, worker, input.prompt, 'work', roleResults, runner, plan.result, state);
  ...
  const final = await runRole(team, lead, input.prompt, 'finalize', roleResults, runner, undefined, state);
...
async function executeJudgeRoute(
  ...
  const roleResults: AgentTeamRoleResult[] = [];
  const events: AgentTeamExecutionEvent[] = [];
  if (!judge) return summarize(team, 'error', roleResults, events, state, 'team has no judge role');

  const judgeResult = await runRole(team, judge, input.prompt, 'judge', roleResults, runner, undefined, state);
  roleResults.push(judgeResult);
  events.push({ kind: 'role', roleId: judge.id, phase: 'judge' });
  if (judgeResult.status === 'error') return summarize(team, 'error', roleResults, events, state, judgeResult.error || judgeResult.result);

  const decision = parseRouteDecision(judgeResult.result, candidates);
  const selected = (decision?.action === 'run_role' ? candidates.find((role) => role.id === decision.target) : null)
  ...
  if (selected) {
    const label = decision
      ...
      : 'Judge 选择路径';
    events.push({ kind: 'route', fromRoleId: judge.id, toRoleId: selected.id, label });
    emitTrace(state, {
      actor: judge.id,
      type: 'route.decided',
      ...
      payload: {
        ...
        target: selected.id,
        reason: decision?.reason ?? 'fallback route target parsed from judge output',
        confidence: decision?.confidence ?? 0.5,
      },
    });
    const routed = await runRole(team, selected, input.prompt, 'route', roleResults, runner, judgeResult.result, state);
    roleResults.push(routed);
    events.push({ kind: 'role', roleId: selected.id, phase: 'route' });
    if (routed.status === 'error') return summarize(team, 'error', roleResults, events, state, routed.error || routed.result);
  }
  ...
  if (finalRole && finalRole.id !== selected?.id) {
    ...
    const final = await runRole(team, finalRole, input.prompt, 'finalize', roleResults, runner, undefined, state);
    roleResults.push(final);
...
async function runRole(
  ...
  if (state) {
    emitTrace(state, {
      ...
      taskId,
      payload: { roleId: role.id, roleName: role.name, phase, status: output.status, error: output.error },
    });
  }
  return {
    roleId: role.id,
...
function summarize(
  ...
  return { status, finalResult, roleResults, events, runId: state.runId, traceId: state.traceId, traceEvents: state.traceEvents, error };
}
...
function createExecutionState(input: AgentTeamExecutionInput): ExecutionState {
  ...
  return { runId, traceId, sessionId: input.sessionId, traceEvents: [], spanSeq: 0 };
}

function emitTrace(
  state: ExecutionState,
  event: Omit<AgentTeamTraceEvent, 'traceId' | 'spanId' | 'runId' | 'sessionId' | 'timestamp' | 'schemaVersion'>,
): void {
  state.spanSeq += 1;
...
function parseRouteDecision(result: string, candidates: AgentTeamRole[]): RouteDecision | null {
  const jsonText = extractJsonObject(result);
  if (!jsonText) return null;
  try {
    const parsed = JSON.parse(jsonText) as Partial<RouteDecision>;
    const action = parsed.action;
    if (!action || !['run_role', 'finish', 'request_approval', 'abort'].includes(action)) return null;
    const confidence = Number(parsed.confidence);
    if (!Number.isFinite(confidence) || confidence < 0 || confidence > 1) return null;
    const target = typeof parsed.target === 'string' ? parsed.target.trim() : undefined;
    if (action === 'run_role' && (!target || !candidates.some((role) => role.id === target))) return null;
    return {
      action,
      target,
      reason: typeof parsed.reason === 'string' && parsed.reason.trim() ? parsed.reason.trim() : 'Judge route decision',
      confidence,
    };
  } catch {
    return null;
