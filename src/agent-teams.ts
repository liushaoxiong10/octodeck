...
export interface AgentTeamGenerationResult {
  ...
  agentMdDefinitionsToCreate: Omit<AgentMdDefinitionInput, 'createdByAgentId'>[];
}
...
const AGENT_MD_FILE = path.join(DATA_DIR, 'config', 'agent-md-definitions.json');
...
function normalizeTeamInput(input: AgentTeamInput): AgentTeamInput {
  const roles = (Array.isArray(input.roles) ? input.roles : [])
    ...
    .map(normalizeRole);
  if (roles.length === 0) {
    roles.push(normalizeRole({ id: 'lead', name: 'Lead', responsibility: '理解目标并协调团队产出。' }, 0));
  }
  return {
    ...
    roles,
    workflow: input.workflow?.trim() || '团队按角色职责协作，并在完成后汇总结果。',
...
function normalizeAgentMdInput(input: AgentMdDefinitionInput): AgentMdDefinitionInput {
  return {
    name: input.name?.trim() || 'Untitled agent.md',
    summary: input.summary?.trim() || '未提供简介。',
    content: input.content?.trim() || '# Untitled agent.md\n\n请在这里定义该角色的职责、工作方式和边界。',
    createdByAgentId: input.createdByAgentId?.trim() || 'unknown',
    createdByUserId: input.createdByUserId?.trim() || undefined,
  };
}
...
function readFile(): StoredAgentTeamsFile {
  ...
  try {
    const parsed = JSON.parse(fs.readFileSync(AGENT_TEAMS_FILE, 'utf-8')) as StoredAgentTeamsFile;
...
function readAgentMdFile(): StoredAgentMdDefinitionsFile {
  ...
  if (stored) {
    try {
      ...
      if (parsed.version === 1 && Array.isArray(parsed.definitions)) return parsed;
    } catch {
      // Fall back to legacy file below.
    }
  }
  ...
  try {
    const parsed = JSON.parse(fs.readFileSync(AGENT_MD_FILE, 'utf-8')) as StoredAgentMdDefinitionsFile;
...
function writeFile(teams: AgentTeam[]): void {
  ...
  const payload: StoredAgentTeamsFile = { version: 1, teams, updatedAt: nowIso() };
...
function writeAgentMdFile(definitions: AgentMdDefinition[]): void {
  ...
  const payload: StoredAgentMdDefinitionsFile = { version: 1, definitions, updatedAt: nowIso() };
...
function belongsToUser(item: { createdByUserId?: string }, ownerUserId?: string): boolean {
  return !ownerUserId || item.createdByUserId === ownerUserId;
}

export function listAgentTeams(ownerUserId?: string): AgentTeam[] {
  return readFile().teams
    .filter((team) => belongsToUser(team, ownerUserId))
    .sort((a, b) => b.updatedAt.localeCompare(a.updatedAt));
}

export function getAgentTeam(id: string, ownerUserId?: string): AgentTeam | null {
  return readFile().teams.find((team) => team.id === id && belongsToUser(team, ownerUserId)) ?? null;
}

export function createAgentTeam(input: AgentTeamInput, ownerUserId?: string): AgentTeam {
...
export function updateAgentTeam(id: string, patch: AgentTeamPatch, ownerUserId?: string): AgentTeam | null {
  const file = readFile();
  const existing = file.teams.find((team) => team.id === id && belongsToUser(team, ownerUserId));
...
export function deleteAgentTeam(id: string, ownerUserId?: string): boolean {
  const file = readFile();
  const next = file.teams.filter((team) => team.id !== id || !belongsToUser(team, ownerUserId));
  if (next.length === file.teams.length) return false;
  writeFile(next);
  return true;
}

export function listAgentMdDefinitions(ownerUserId?: string): AgentMdDefinition[] {
  return readAgentMdFile().definitions
    .filter((definition) => belongsToUser(definition, ownerUserId))
    .sort((a, b) => b.updatedAt.localeCompare(a.updatedAt));
}

export function listAgentMdSummaries(ownerUserId?: string): AgentMdSummary[] {
  return listAgentMdDefinitions(ownerUserId).map(({ id, name, summary }) => ({ id, name, summary }));
}

export function getAgentMdDefinition(id: string, ownerUserId?: string): AgentMdDefinition | null {
  return readAgentMdFile().definitions.find((definition) => definition.id === id && belongsToUser(definition, ownerUserId)) ?? null;
}

export function createAgentMdDefinition(input: AgentMdDefinitionInput, ownerUserId?: string): AgentMdDefinition {
...
export function updateAgentMdDefinition(id: string, patch: AgentMdDefinitionPatch, ownerUserId?: string): AgentMdDefinition | null {
  const file = readAgentMdFile();
  const existing = file.definitions.find((definition) => definition.id === id && belongsToUser(definition, ownerUserId));
  if (!existing) return null;
  ...
  writeAgentMdFile(file.definitions.map((definition) => (definition.id === id ? updated : definition)));
  return updated;
}

export function deleteAgentMdDefinition(id: string, ownerUserId?: string): boolean {
  const file = readAgentMdFile();
  const next = file.definitions.filter((definition) => definition.id !== id || !belongsToUser(definition, ownerUserId));
  if (next.length === file.definitions.length) return false;
  writeAgentMdFile(next);
  return true;
}

export function buildAgentTeamDraft(input: AgentTeamDraftInput): AgentTeamInput {
...
export function buildAgentTeamGenerationPrompt(
  ...
  const summaryBlock = agentMdSummaries.length
    ? agentMdSummaries
        .slice(0, 30)
        .map((definition) => `- ${definition.name} (${definition.id}): ${definition.summary}`)
        .join('\n')
    : '- 暂无现有 agent.md 定义。';
...
function inferTeamName(goal: string, shape: AgentTeamShape): string {
  const prefix = goal.replace(/[\n\r\t]+/g, ' ').trim().slice(0, 24) || 'Agent';
...
function rolesForShape(shape: AgentTeamShape): AgentTeamRole[] {
  if (shape === 'parallel') {
    return [
      role('researcher', 'Research Agent', '从事实、资料和约束角度并行收集信息。', ['用户目标'], ['事实清单', '风险假设']),
      role('designer', 'Solution Agent', '从方案和架构角度并行提出可选路径。', ['用户目标'], ['方案选项', '取舍说明']),
      role('synthesizer', 'Synthesis Agent', '合并并行结果，输出一致结论。', ['事实清单', '方案选项'], ['最终建议']),
    ];
  }
  if (shape === 'leader-worker') {
    return [
      role('lead', 'Lead Agent', '拆解目标、分配任务并整合 worker 结果。', ['用户目标'], ['任务拆解', '最终交付']),
      role('worker_analysis', 'Analysis Worker', '负责分析背景、约束和风险。', ['任务拆解'], ['分析结果']),
      role('worker_delivery', 'Delivery Worker', '负责形成可执行交付物。', ['任务拆解', '分析结果'], ['交付草案']),
    ];
  }
  if (shape === 'judge-route') {
    return [
      role('judge', 'Judge Agent', '评估当前状态并决定下一步路由。', ['用户目标', '阶段产出'], ['路由决策', '质量意见']),
      role('executor', 'Executor Agent', '根据 judge 路由执行具体任务。', ['路由决策'], ['阶段产出']),
      role('reviewer', 'Review Agent', '检查产出质量并给出修正建议。', ['阶段产出'], ['评审结果']),
    ];
  }
  return [
    role('planner', 'Planning Agent', '理解目标并拆解阶段。', ['用户目标'], ['执行计划']),
    role('builder', 'Execution Agent', '按计划推进主要工作。', ['执行计划'], ['阶段交付']),
    role('reviewer', 'Review Agent', '验证结果并整理最终交付。', ['阶段交付'], ['最终交付']),
  ];
}

function role(id: string, name: string, responsibility: string, inputs: string[], outputs: string[]): AgentTeamRole {
  return {
    id,
    name,
    responsibility,
...
function workflowForShape(shape: AgentTeamShape): string {
  if (shape === 'parallel') return 'Parallel：多个角色同时处理不同视角，最后由 Synthesis Agent 合并结果。';
  if (shape === 'leader-worker') return 'Leader-worker：Lead Agent 负责拆解、协调和整合，Worker Agent 分别完成分析与交付。';
  if (shape === 'judge-route') return 'Judge route：Judge Agent 在每个阶段评估质量并决定由哪个角色继续处理。';
  if (shape === 'pipeline') return 'Pipeline：角色按顺序接力，每一步消费上一步输出并交付给下一步。';
