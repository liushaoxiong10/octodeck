...
export function SystemSettingsSection() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 禁用 OctoDeck 记忆层（admin 主容器专用） */}
      <div className="border-t border-border pt-6 space-y-5">
        <h3 className="text-sm font-semibold text-foreground">禁用 OctoDeck 记忆层（admin 主容器）</h3>
        ...
        <div className="flex items-center justify-between">
          <div className="flex-1 pr-4">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              ...
              不注入 WORKSPACE_GLOBAL/MEMORY 环境变量、不注入 OctoDeck 记忆系统提示、
            ...
            <p className="text-xs text-destructive mt-2">
              ...
              SDK 既读不到 OctoDeck 记忆层也读不到 ~/.claude/，Agent 会变成空白沙箱。
              后端会在保存时校验，未配置则拒绝启用。
            </p>
            <p className="text-xs text-muted-foreground mt-2">
            ...
            <p className="text-xs text-muted-foreground mt-2">
              ...
              如果 OctoDeck Agent 和你本人的风格/语言偏好不同，两边会互相污染 —
              介意就不要开这个开关。
            </p>
          </div>
          <Switch
