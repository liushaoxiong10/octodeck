...
function DetailRow({ label, value }: { label: string; value: string | number | null | undefined }) {
  return (
    <div className="rounded-xl border border-border/70 bg-background/70 px-3 py-2">
      ...
      <div className="mt-1 break-all text-xs font-medium text-foreground">{value || '—'}</div>
    </div>
  );
}
  ...
  return (
    <div className="rounded-xl border border-border/70 bg-background/70 p-3">
      <div className="flex items-center justify-between gap-3">
        <div className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">{label}</div>
        <div className="text-xs font-semibold text-foreground">{percent == null ? '—' : `${value.toFixed(1)}%`}</div>
      </div>
      <Progress className="mt-2 h-2" value={value} role="progressbar" aria-label={`${label} usage`} />
...
function ServingAgents({ agents }: { agents: CustomBackendDef[] }) {
  if (agents.length === 0) {
    return <EmptyBlock>暂无绑定到该设备的 Agent。可在 Agents 页面基于该设备创建 Agent。</EmptyBlock>;
  }
  return (
    <div className="space-y-2">
      {agents.map((agent) => (
        <div key={agent.id} className="rounded-xl border border-border bg-background/80 p-3">
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <div className="truncate text-sm font-medium text-foreground">{agent.displayName}</div>
              <div className="mt-0.5 truncate text-xs text-muted-foreground">
                {agent.id} · {agent.agentClientId ?? 'manual'} · {agent.outputProtocol}
              </div>
            </div>
...
function DeviceDetails({
  ...
  const agentClientNames = link.agentClients.map((c) => c.displayName).join('、');
  const permissionModes = uniqueValues(link.agentClients.map((c) => c.permissionModes));
  const providerCapabilities = uniqueValues(link.agentClients.map((c) => c.capabilities));
    ...
    <div className="space-y-4">
        ...
        <div className="relative flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            <div className="mb-2 flex items-center gap-2">
              <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-medium ${statusTone(link)}`}>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              {link.hostname ?? 'unknown host'} · {link.os ?? '?'} / {link.arch ?? '?'}
            </p>
          </div>
          <div className="grid grid-cols-2 gap-2 sm:min-w-64">
            <div className="rounded-2xl border border-border/70 bg-background/80 p-3">
              <div className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">在线时长</div>
              <div className="mt-1 text-sm font-semibold text-foreground">{uptime}</div>
            </div>
            <div className="rounded-2xl border border-border/70 bg-background/80 p-3">
              <div className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Agents</div>
              <div className="mt-1 text-sm font-semibold text-foreground">{servingAgents.length}</div>
            </div>
          </div>
        </div>
        <div className="relative mt-4 flex flex-wrap gap-2">
          <Button size="sm" variant="outline" onClick={() => onRotate(link)} disabled={busy}>
            {busy ? <Loader2 className="size-4 animate-spin" /> : <RefreshCw className="size-4" />}
            重置 token
          </Button>
          <Button size="sm" variant="outline" onClick={() => onRemove(link)} disabled={busy} className="text-destructive hover:text-destructive">
      ...
      <details className="group rounded-2xl border border-border bg-card shadow-sm">
        ...
        <div className="grid gap-2 border-t border-border p-4 sm:grid-cols-2">
          ...
          <DetailRow label="Start time" value={formatTime(link.lastConnectedAt)} />
          <DetailRow label="Last heartbeat" value={formatTime(link.lastSeenAt)} />
          ...
          <DetailRow label="Client version" value={link.clientVersion ? `v${link.clientVersion}` : null} />
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        ...
        <DeviceInfoSection icon={Layers3} title="Capabilities">
          {providerCapabilities.length > 0 ? (
            <div className="space-y-3">
              <div className="text-xs text-muted-foreground">provider CLI capability matrix</div>
              {link.agentClients.map((client) => (
                <div key={client.id} className="rounded-xl border border-border bg-background/80 p-3">
                  <div className="text-xs font-medium text-foreground">{client.displayName}</div>
                  <div className="mt-2 flex flex-wrap gap-2">
                    {(client.capabilities ?? []).map((capability) => (
                      <Pill key={`${client.id}-${capability}`}>{capability}</Pill>
                    ))}
                  </div>
                </div>
              ))}
        ...
        <DeviceInfoSection icon={TerminalSquare} title="CLI Providers">
          {link.agentClients.length > 0 ? (
            <div className="space-y-2">
              {link.agentClients.map((client) => (
                <div key={client.id} className="rounded-xl border border-border bg-background/80 p-3 text-xs">
                  <div className="font-medium text-foreground">{client.displayName}</div>
                  <div className="mt-1 break-all text-muted-foreground">
                    {client.id} · {client.binary}{client.version ? ` · ${client.version}` : ''}
                  </div>
                </div>
              ))}
            </div>
          ...
          {permissionModes.length > 0 ? (
            <div className="space-y-3">
              ...
              <div className="text-xs leading-5 text-muted-foreground">
                Permission modes 指 default、acceptEdits 等 provider CLI 自身支持的权限模式。
              </div>
            </div>
          ) : (
        ...
        <DeviceInfoSection icon={Sparkles} title="Local skills">
          <EmptyBlock>当前协议尚未上报本地 skills；后续可在 hello frame 中扩展。</EmptyBlock>
        </DeviceInfoSection>
        ...
        <DeviceInfoSection icon={Boxes} title="Resources">
          <div className="grid gap-2 sm:grid-cols-2">
            ...
            <DetailRow label="Collected at" value={formatTime(link.resources?.collectedAt ?? null)} />
        ...
        <DeviceInfoSection icon={Wrench} title="Diagnostics">
          <div className="grid gap-2">
            <DetailRow label="Connection" value={link.online ? 'online' : 'offline'} />
            <DetailRow label="Heartbeat" value={formatTime(link.lastSeenAt)} />
            <DetailRow label="Version" value={link.clientVersion ? `v${link.clientVersion}` : 'unknown'} />
          </div>
        </DeviceInfoSection>
      </div>
    </div>
...
export function DevicesSection() {
  const { links, loading, load, create, rotate, remove } = useAgentLinksStore();
  ...
  const handleRotate = async (link: AgentLink) => {
    if (!window.confirm(`确认重置 "${link.displayName}" 的接入 token？旧 token 将立即失效。`)) return;
    setBusyId(link.id);
    try {
  ...
  const handleRemove = async (link: AgentLink) => {
    if (!window.confirm(`确认移除 "${link.displayName}"？该设备将立即断连且无法重连。`)) return;
    setBusyId(link.id);
    try {
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <span className="truncate text-sm font-medium text-foreground">{l.displayName}</span>
                          </div>
                          <div className="mt-1 truncate text-xs text-muted-foreground">
                            {l.hostname ?? '—'} · {l.os ?? '?'} / {l.arch ?? '?'}
                          </div>
                          <div className="mt-1 text-[11px] text-muted-foreground">
                            {l.online ? `在线 ${formatDurationSince(l.lastConnectedAt, true)}` : `心跳 ${formatTime(l.lastSeenAt)}`}
                          </div>
                        </div>
                        <span className={`rounded-full px-2 py-0.5 text-[10px] ${l.online ? 'bg-emerald-500/15 text-emerald-700 dark:text-emerald-300' : 'bg-muted text-muted-foreground'}`}>
                          {l.online ? 'on' : 'off'}
                        </span>
                      </div>
                    </button>
        ...
        <DialogContent>
          ...
          {tokenDialog && (
            <div className="space-y-2">
              ...
              <div className="text-xs text-muted-foreground mt-3">一键安装命令</div>
              ...
              <p className="text-[11px] text-muted-foreground leading-5">
                命令格式为 curl -fsSL &quot;{getCurrentServerOrigin()}/api/daemon/install-script?...&quot; | bash。
                请只在你信任的机器上执行。
              </p>
            </div>
          )}
