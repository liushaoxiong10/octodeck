...
export const MessageBubble = memo(
  function MessageBubble({
    ...
    const isOtherUser =
      isShared && isUser && message.sender !== currentUser?.id;
    ...
    if (message.role === 'tool') {
      ...
      const summary = summarizeToolMessage(message.content);
      return (
        <div className="group mb-3 flex justify-center">
          <div className="w-full max-w-3xl overflow-hidden rounded-xl border border-blue-200/70 bg-blue-50/50 text-sm dark:border-blue-900/60 dark:bg-blue-950/20">
            <div className="flex min-h-9 items-center gap-2 px-3 py-2 text-xs font-medium text-blue-700 dark:text-blue-300">
              <Wrench className="h-3.5 w-3.5" />
              <span className="shrink-0">tool · {title}</span>
              <span
                className="min-w-0 flex-1 truncate font-normal text-blue-950/75 dark:text-blue-100/75"
                title={summary}
              >
                {summary}
              </span>
              ...
              <button
                ...
                className="inline-flex h-6 shrink-0 items-center gap-0.5 rounded px-1.5 text-[11px] text-muted-foreground hover:bg-blue-100/70 hover:text-blue-700 dark:hover:bg-blue-900/40 dark:hover:text-blue-200"
            ...
            {toolExpanded && (
              <div className="border-t border-blue-200/70 px-3 py-2 dark:border-blue-900/60">
                <pre className="max-h-72 overflow-auto whitespace-pre-wrap break-words rounded-md bg-background/60 px-2 py-1.5 text-xs leading-relaxed text-blue-950/80 dark:text-blue-100/80">
                  {message.content}
                </pre>
                <button
                  onClick={handleCopy}
