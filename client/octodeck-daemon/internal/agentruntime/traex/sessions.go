// Package traex — provider session metadata helper.
//
...
// ProviderDir is the on-disk directory name used by the TraeX CLI to
...
const ProviderDir = "traex"

// ListSessions enumerates persisted TraeX sessions for a given agent
// client (traex or traex-acp) under the requested workspace. An empty
// workspace string scans every workspace.
//
// agentClientID is the daemon-level agent ID (e.g. "traex-acp") and is
...
// DeleteSession removes a single TraeX provider session. It first tries
