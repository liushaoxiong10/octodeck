// Package claudecode — provider session metadata helper.
//
...
// ProviderDir is the on-disk directory name used by the Claude CLI to
...
// ListSessions enumerates persisted Claude sessions for a given agent
// client (claude-code or claude-acp) under the requested workspace. An
// empty workspace string scans every workspace.
//
// agentClientID is the daemon-level agent ID (e.g. "claude-code") and is
...
// DeleteSession removes a single Claude provider session. It first tries
