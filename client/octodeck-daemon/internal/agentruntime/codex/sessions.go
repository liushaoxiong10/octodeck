// Package codex — provider session metadata helper.
//
...
// ProviderDir is the on-disk directory name used by the Codex CLI to
...
const ProviderDir = "codex"

// ListSessions enumerates persisted Codex sessions for a given agent
// client (codex or codex-acp) under the requested workspace. An empty
// workspace string scans every workspace.
//
// agentClientID is the daemon-level agent ID (e.g. "codex") and is used
// to resolve metadata located under the agent-id sub-directory in
...
// DeleteSession removes a single Codex provider session. It first tries
// the family provider directory and, if nothing was deleted, falls back
// to the agent-id sub-directory that BaseAgent historically maintained
// for ID-scoped metadata.
