...
// capability.go）认识各 family，不再需要硬编码 claude / codex / traecli /
// traex 等字面量。
//
...
type Descriptor struct {
	...
	DisplayName string
	// Family 是这一类 agent 的归属（claude / codex / traecli / traex / 自定义）。
	Family string
	...
	// 同一个可执行文件通过子命令暴露不同 transport 的场景（如 traecli acp）。
	Args []string
